...
export default async function generateTicketsForEvent(eventId: string) {
  ...
  if (event) {
    const { price, capacity } = event; // Obtenez le prix du ticket et la capacité de l'événement
    const numberOfTickets = capacity; // Utilisez la capacité de l'événement comme nombre de tickets à générer
